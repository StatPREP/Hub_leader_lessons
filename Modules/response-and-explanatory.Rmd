...
PR <- list(name = "proportions", url = "missing")
...
-------------|--------------|-------------------------|-------------
Quantitative | Quantitative |  linear regression      | [`r LR$name`](`r LR$url`)
Quantitative | Categorical  |  group-wise means       | [`r TT$name`](`r TT$url`)
Categorical  | Quantitative |  proportion regression  | [`r PR$name`](`r PR$url`)
Categorical  | Categorical  |  group-wise proportions | [`r PR$name`](`r PR$url`)
...
1.View the data with a point plot, using the [`r point_plot_app$name`](`r point_plot_app$url`) Little App. This will let you readily see which type each variable is. (The choice of one variable as response and one as explanatory isn't critical here, since the reverse choice would generate the same plot but turned around the diagonal to reverse the axes.)
...
* `diamonds`
    - The price of a diamond and its weight in carets.
