...
lesson_format_string <- function(base_name) {
  glue::glue("Formats: [web](Lessons/{base_name}.html) : [pdf](Lessons/{base_name}.pdf) : [word](Lessons/{base_name}.docx) : [Rmd](Lessons/{base_name}.Rmd)")
}
```

These *Topics for Stat 101* generally involve computing which is provided entirely through mouse-driven, web-based "Little Apps". Using these Little Apps requires an ordinary web browser and nothing more.
